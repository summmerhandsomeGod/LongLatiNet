...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Segment, IDetect)):
            s = 256  # 2x min stride
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                ScConv, C3EMA1, C3EMA2, C3EMA3, C2f, Shake, Shake1, Shake2, DCnv2, Shake3, AdaptiveShake,
                CoSima, StarConv, RecursiveBottleneck, SPPF_3_9_9}:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x, C2f, RecursiveBottleneck}:
        ...
        # TODO: channel, gw, gd
        elif m in {Detect, Segment, IDetect, IAuxDetect}:
